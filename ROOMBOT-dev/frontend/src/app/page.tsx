...
export default function Home() {
  ...
  useEffect(() => {
    if (user && guestId) {
      // merge API 호출
      fetch('http://localhost:8000/protected/merge-guest-data', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${user.getIdToken ? user.getIdToken() : ''}`
        },
        body: JSON.stringify({ guest_id: guestId, merge_strategy: 'replace' })
      })
      .then(res => res.json())
      .then(data => {
        // merge 성공 시 guest_id 초기화(삭제)
        localStorage.removeItem('guest_id');
        setGuestId(null);
      })
      .catch(err => {
        // 에러 무시(게스트 데이터 없을 수도 있음)
      });
    }
  }, [user, guestId]);
