...
> ROOMBOT은 GPT가 생성한 문장을 외부 정보와 비교 분석해 사실/부분사실/환각 여부를 판별하고, 그 결과를 시각화 및 리포트로 제공합니다.
...
* SBERT 유사도 분석을 통한 환각 판단 (사실/부분사실/환각)
* 문장별 결과 시각화 (색상/차트)
...
| 주제 | 이유 |
| --- | --- |
| 한국 역사 | 연도, 인물, 사건 오류가 잦으며 위키 기반 검증이 용이함 |
| 과학 발명 및 노벨상 | 발명 시점 및 수상 연도 착오가 자주 발생하며 공식 목록으로 검증 가능 |
| 스포츠 메달 기록 | 연도, 선수, 종목 오류가 잦으며 Wikipedia 및 Google로 빠른 검증 가능 |
...
| 입력 | GPT 응답 | 환각 탐지 결과 |
| --- | --- | --- |
| "세종대왕은 1392년에 조선을 세웠다" | (GPT 응답 출력) | [환각] 유사도 0.31 로 분류됨 |
