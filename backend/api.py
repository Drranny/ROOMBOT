...
# FastAPI 앱 초기화
app = FastAPI(
    title="SBERT Sentence Similarity API",
    description="SBERT를 이용한 문장 유사도 계산 API",
    version="1.0.0"
)

# SBERT 계산기 초기화 (다국어 지원 모델 사용)
...
async def root():
    """API 루트 엔드포인트"""
    return {
        "message": "SBERT Sentence Similarity API",
        "version": "1.0.0",
        "endpoints": {
...
async def calculate_similarity(request: SimilarityRequest):
    ...
    Args:
        request: SimilarityRequest - 계산할 두 문장과 모델명
        
    Returns:
        SimilarityResponse: 유사도 계산 결과
    """
    global calculator
    try:
        # 모델이 다르면 새로운 계산기 생성
        model_name = request.model_name if request.model_name is not None else 'paraphrase-multilingual-MiniLM-L12-v2'
...
async def calculate_batch_similarity(request: BatchSimilarityRequest):
    ...
    Args:
        request: BatchSimilarityRequest - 문장 쌍들의 리스트와 모델명
        
    Returns:
        List[Dict]: 각 문장 쌍의 유사도 결과 리스트
    """
    global calculator
    try:
        # 모델이 다르면 새로운 계산기 생성
...
async def find_most_similar(request: FindSimilarRequest):
    ...
    Args:
        request: FindSimilarRequest - 쿼리 문장, 후보 문장들, top_k, 모델명
