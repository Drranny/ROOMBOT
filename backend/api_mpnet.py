...
# FastAPI 앱 초기화
app = FastAPI(
    title="SBERT MPNet Sentence Similarity API",
    description="paraphrase-mpnet-base-v2 모델을 이용한 문장 유사도 계산 API",
    version="1.0.0"
)

# SBERT 계산기 초기화 (paraphrase-mpnet-base-v2 모델)
calculator = SentenceSimilarityCalculator('paraphrase-mpnet-base-v2')

# Pydantic 모델들
class SimilarityRequest(BaseModel):
    sentence1: str
    sentence2: str
    model_name: Optional[str] = 'paraphrase-mpnet-base-v2'
...
async def root():
    """API 루트 엔드포인트"""
    return {
        "message": "SBERT MPNet Sentence Similarity API",
        "version": "1.0.0",
...
async def calculate_similarity(request: SimilarityRequest):
    ...
    global calculator
    try:
        model_name = request.model_name if request.model_name is not None else 'paraphrase-mpnet-base-v2'
...
async def health_check():
    """헬스 체크 엔드포인트"""
    return {"status": "healthy", "model_loaded": True, "model": "paraphrase-mpnet-base-v2"}
