...
# FastAPI 앱 초기화
app = FastAPI(
    title="SBERT T5 Sentence Similarity API",
    description="sentence-t5-base/large 모델을 이용한 문장 유사도 계산 API",
    version="1.0.0"
)

# SBERT 계산기 초기화 (sentence-t5-base 모델)
calculator = SentenceSimilarityCalculator('sentence-t5-base')

# Pydantic 모델들
class SimilarityRequest(BaseModel):
    sentence1: str
    sentence2: str
    model_name: Optional[str] = 'sentence-t5-base'
...
async def root():
    """API 루트 엔드포인트"""
    return {
        "message": "SBERT T5 Sentence Similarity API",
        "version": "1.0.0",
        "default_model": "sentence-t5-base",
        "supported_models": ["sentence-t5-base", "sentence-t5-large"],
        "endpoints": {
...
async def calculate_similarity(request: SimilarityRequest):
    ...
    global calculator
    try:
        model_name = request.model_name if request.model_name is not None else 'sentence-t5-base'
        if model_name not in ['sentence-t5-base', 'sentence-t5-large']:
            raise HTTPException(status_code=400, detail="지원하지 않는 모델입니다. 'sentence-t5-base' 또는 'sentence-t5-large'를 사용하세요.")
...
async def health_check():
    """헬스 체크 엔드포인트"""
    return {"status": "healthy", "model_loaded": True, "model": calculator.model_name}
